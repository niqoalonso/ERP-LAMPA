...
class EncabezadoDocumentoController extends Controller
{
    ...
    {   
        ...
        InfoDocumento::create([ 'n_documento' => (!empty($cod)) ? $cod->n_documento+1 : 1, 'glosa' => $request->glosa, 'documento_id' => $request->documento_id,
                                'fecha_emision' => $request->fechadoc, 'fecha_vencimiento' => $request->fechaven,'empresa_id' => $request->empresa['id_empresa'],
                                'estado_id' => 12, 'encabezado_id' => $encebezado->id_encabezado]);
        
        return  $request->n_documento;
    }
